...
public abstract class BaseItemSelectPanel<T extends Item> {
    @ApiModelProperty(value = "可选面板集合")
    private List<T> itemList;
    @ApiModelProperty(value = "列表元素是否可上下移动")
    private boolean movable = false;
    @ApiModelProperty(value = "左边面板")
    private JPanel leftPanel;
    @ApiModelProperty(value = "右边面板")
    private JPanel rightPanel;
    @ApiModelProperty(value = "列表面板")
    ...
    public JComponent getComponent() {
        ...
        ActionToolbar actionToolbar = ActionManager.getInstance().createActionToolbar("Item Toolbar", actionGroup,
                true);
    ...
    private String inputItemName(String initValue) {
        return Messages.showInputDialog(MsgValue.ITEM_NAME_LABEL, MsgValue.TITLE_INFO, Messages.getQuestionIcon(),
                initValue, new InputValidator() {
            @Override
    ...
    private DefaultActionGroup createActionGroup() {
        ...
        actionGroup.add(new AnAction(AllIcons.General.Remove) {
            ...
            public void actionPerformed(@NotNull AnActionEvent e) {
                T selectedItem = getSelectedItem();
                // 确认删除？
                if (MessageDialogBuilder.yesNo(MsgValue.TITLE_INFO, String.format(MsgValue.CONFIRM_DELETE_MESSAGE,
                        selectedItem.getName())).isYes()) {
