...
import ltd.kafka.soonelu.plugins.ui.gen.base.BaseGroupPanel;
import ltd.kafka.soonelu.plugins.ui.gen.base.BaseItemSelectPanel;
...
public class TemplateSettingPanel implements Configurable {
    @ApiModelProperty(value = "模板描述信息，说明文档")
    private static final String TEMPLATE_DESCRIPTION_INFO;

    static {
        String descriptionInfo = "";
        try {
            descriptionInfo = UrlUtil.loadText(TemplateSettingPanel.class.getResource("/description" +
    ...
    @ApiModelProperty(value = "编辑框面板")
    private TemplateEditor templateEditor;

    @ApiModelProperty(value = "基本的分组面板")
    private BaseGroupPanel baseGroupPanel;

    @ApiModelProperty(value = "基本的元素选择面板")
    private BaseItemSelectPanel<Template> baseItemSelectPanel;

    @ApiModelProperty(value = "当前分组")
    ...
    TemplateSettingPanel() {
        ...
        // 配置服务实例化
        this.genSettings = GenSettings.getInstance();
        // 克隆对象
        this.currGroupName = this.genSettings.getCurrTemplateGroupName();
        this.group = CloneUtils.cloneByJson(this.genSettings.getTemplateGroupMap(), new TypeReference<Map<String,
                TemplateGroup>>() {
        });
    }
        ...
        this.baseItemSelectPanel = new BaseItemSelectPanel<Template>(group.get(currGroupName).getElementList(), true) {
            ...
            protected void selectedItem(Template item) {
                ...
                if (templateEditor == null) {
                    ...
                    templateEditor = new TemplateEditor(project, item.getName() + ".vm", item.getCode(),
                            TEMPLATE_DESCRIPTION_INFO, velocityFileType);
                    // 代码修改回调
    ...
    private void addDebugPanel() {
        ...
        DefaultActionGroup actionGroup = new DefaultActionGroup(new AnAction(AllIcons.Debugger.Console) {
            ...
            public void actionPerformed(@NotNull AnActionEvent e) {
                ...
                // 生成代码
                String code = CodeGenerateService.getInstance(project).generate(new Template("temp",
                        templateEditor.getEditor().getDocument().getText()), tableInfo);
                ...
                FileType velocityFileType = FileTypeManager.getInstance().getFileTypeByExtension("vm");
                PsiFile psiFile = psiFileFactory.createFileFromText("EasyCodeTemplateDebug.vm.ft", velocityFileType,
                        code, 0, true);
        ...
        ActionToolbar actionToolbar = ActionManager.getInstance().createActionToolbar("Template Debug", actionGroup,
                true);
    ...
    public boolean isModified() {
        return !genSettings.getTemplateGroupMap().equals(group) || !genSettings.getCurrTemplateGroupName().equals(currGroupName);
    }
    ...
    @Override
    public void apply() {
        genSettings.setTemplateGroupMap(group);
        genSettings.setCurrTemplateGroupName(currGroupName);
    }
    ...
    public void reset() {
        ...
        // 防止对象篡改，需要进行克隆
        this.group = CloneUtils.cloneByJson(genSettings.getTemplateGroupMap(), new TypeReference<Map<String,
                TemplateGroup>>() {
        });
        this.currGroupName = genSettings.getCurrTemplateGroupName();
