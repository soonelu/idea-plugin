...
import ltd.kafka.soonelu.plugins.entity.gen.GlobalConfig;
...
public class GlobalConfigSettingPanel implements Configurable {
    @ApiModelProperty(value = "全局配置描述信息，说明文档")
    ...
    GlobalConfigSettingPanel() {
        ...
        // 克隆对象
        this.currGroupName = this.genSettings.getCurrGlobalConfigGroupName();
        this.group = CloneUtils.cloneByJson(this.genSettings.getGlobalConfigGroupMap(), new TypeReference<Map<String,
                GlobalConfigGroup>>() {
        });
    }
        ...
        this.baseItemSelectPanel = new BaseItemSelectPanel<GlobalConfig>(group.get(currGroupName).getElementList()) {
            ...
            protected void selectedItem(GlobalConfig item) {
                ...
                if (templateEditor == null) {
                    ...
                    templateEditor = new TemplateEditor(project, item.getName() + ".vm", item.getValue(),
                            GLOBAL_CONFIG_DESCRIPTION_INFO, velocityFileType);
                    // 代码修改回调
    ...
    public boolean isModified() {
        return !genSettings.getGlobalConfigGroupMap().equals(group) || !genSettings.getCurrGlobalConfigGroupName().equals(currGroupName);
    }
    ...
    @Override
    public void apply() {
        genSettings.setGlobalConfigGroupMap(group);
        genSettings.setCurrGlobalConfigGroupName(currGroupName);
    }
    ...
    public void reset() {
        ...
        // 防止对象篡改，需要进行克隆
        this.group = CloneUtils.cloneByJson(genSettings.getGlobalConfigGroupMap(), new TypeReference<Map<String,
                GlobalConfigGroup>>() {
        });
        this.currGroupName = genSettings.getCurrGlobalConfigGroupName();
