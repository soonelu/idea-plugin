package ltd.kafka.soonelu.plugins.service.gson;
...
import ltd.kafka.soonelu.plugins.entity.gson.DataType;
import ltd.kafka.soonelu.plugins.entity.gson.FieldEntity;
import ltd.kafka.soonelu.plugins.entity.gson.IterableFieldEntity;
import ltd.kafka.soonelu.plugins.ui.gson.FieldsDialog;
...
import ltd.kafka.soonelu.plugins.utils.gson.Utils;
...
public class ConvertBridge {
    ...
    public ConvertBridge(Operator operator,
        ...
        String[] arg = GsonConfig.getInstant().getAnnotationStr().replace("{filed}", "(\\w+)").split("\\.");
    ...
    public void run() {
        ...
        if (json != null) {
            try {
                ClassEntity classEntity = collectClassAttribute(targetClass, GsonConfig.getInstant().isReuseEntity());
                if (classEntity != null) {
    ...
    private void collectPackAllClassName() {
        File packageFile = GsonPsiClassUtil.getPackageFile(file, packageName);
        if (packageFile != null) {
    ...
    private String getPackName(PsiClass psiClass) {
    ...
    private void parseJson(JSONObject json) {
        ...
        if (GsonConfig.getInstant().isVirgoMode()) {
    ...
    private void handleNormal(JSONObject json, List<String> generateFiled) {
        WriteCommandAction.runWriteCommandAction(project, () -> {
            if (targetClass == null) {
                try {
                    targetClass = GsonPsiClassUtil.getPsiClass(file, project, generateClassName);
            ...
            if (targetClass != null) {
                ...
                GsonConfig.getInstant().saveCurrentPackPath(packageName);
                operator.dispose();
            }
        });
    }
    ...
    private List<FieldEntity> createFields(JSONObject json, List<String> fieldList, ClassEntity parentClass) {
        ...
        boolean writeExtra = GsonConfig.getInstant().isGenerateComments();
    ...
    private FieldEntity createField(ClassEntity parentClass, String key, Object type) {
        ...
        if (GsonConfig.getInstant().isUseSerializedName()) {
            fieldName = NameUtils.getInstance().getJavaName(convertSerializedName(fieldName));
        }
    ...
    private String convertSerializedName(String fieldName) {
        if (GsonConfig.getInstant().isUseFieldNamePrefix() &&
                !TextUtils.isEmpty(GsonConfig.getInstant().getFiledNamePreFixStr())) {
            fieldName = GsonConfig.getInstant().getFiledNamePreFixStr() + "_" + fieldName;
        }
        return fieldName;
    }
    ...
    private FieldEntity typeByValue(ClassEntity parentClass, String key, Object type) {
        ...
        if (type instanceof JSONObject) {
            ...
            if (classEntity == null) {
                ...
                ClassEntity innerClassEntity = createInnerClass(createSubClassName(key, type), (JSONObject) type,
                        parentClass);
            ...
            fieldEntity.setKey(key);
            String vType;
            if (GsonConfig.getInstant().isUseWrapperClass()) {
...
//                        }
    ...
    private String createSubClassName(String key, Object o) {
        ...
        if (o instanceof JSONObject) {
            ...
            name = stringBuilder.toString() + GsonConfig.getInstant().getSuffixStr();
        }
        return name;
